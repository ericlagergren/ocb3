...
func New(b cipher.Block) (cipher.AEAD, error) {
	return newWithSize(b, defaultNonceSize, defaultTagSize)
}

func newWithSize(b cipher.Block, nonceSize, tagSize int) (cipher.AEAD, error) {
...
func (a *aead) Open(dst, nonce, ciphertext, additionalData []byte) ([]byte, error) {
	...
	ret, out := sliceForAppend(dst, len(ciphertext)-a.tagSize)
...
// It returns a slice with the contents of the given slice
// followed by that many bytes and a second slice that aliases
// into it and contains only the extra bytes. If the original
// slice has sufficient capacity then no allocation is performed.
...
// anyOverlap reports whether x and y share memory at any (not
// necessarily corresponding) index. The memory beyond the slice
// length is ignored.
...
// inexactOverlap reports whether x and y share memory at any
// non-corresponding index. The memory beyond the slice length is
// ignored. Note that x and y can have different lengths and
// still not have any inexact overlap.
//
// InexactOverlap can be used to implement the requirements of
// the crypto/cipher AEAD, Block, BlockMode and Stream
// interfaces.
